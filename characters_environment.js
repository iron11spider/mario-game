...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press play button to start to Start and Play ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if (game_status=="start" && nose_x!="" && gameConfig.status==="start") {
    world_start.play();
...
function jumping(character){
	if( (nose_y<200 &&character.live) || (touchIsDown&&character.live) ){
    mario_jump.play();
